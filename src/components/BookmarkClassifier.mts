import {
  CLASSIFY_BOOKMARKS_SYSTEM_PROMPT,
  GENERATE_CATEGORIES_SYSTEM_PROMPT,
} from "../data/systemPrompts.mjs";
import {
  BOOKMARK_CLASSIFICATION_JSON_SCHEMA,
  GENERATE_CATEGORIES_JSON_SCHEMA,
  createClassificationSchema,
} from "../data/responseConstraintSchemas.mjs";
import {
  showBadgeError,
  showBadgeSuccess,
  startExtensionSpinner,
  resetBadgeToDefault,
...
export default class BookmarkClassifier extends HTMLElement {
  private originalBookmarksMap: Map<string, chrome.bookmarks.BookmarkTreeNode> =
    new Map();
  private allClusteredData: { id: string; cluster_name: string }[] = [];
  private folderHierarchy: FolderHierarchy | null = null;
  private activeSchema: Record<string, any> =
    BOOKMARK_CLASSIFICATION_JSON_SCHEMA;
  ...
  private bindEvents() {
    this.shadowRoot!.querySelector("#create-classifications")!.addEventListener(
      "click",
      this.createClassifications.bind(this),
    );
    this.shadowRoot!.querySelector("#clear-classifications")!.addEventListener(
      "click",
      this.clearClassifications.bind(this),
    );
    this.shadowRoot!.querySelector("#create-hierarchy")!.addEventListener(
      "click",
      this.createFolderHierarchy.bind(this),
    );
    this.shadowRoot!.querySelector("#assign-folders")!.addEventListener(
      "click",
      this.assignBookmarksToFolders.bind(this),
    );
    this.shadowRoot!.querySelector("#generate-novel-schema")!.addEventListener(
      "click",
      this.generateNovelSchema.bind(this),
    );
  }
  ...
  async generateNovelSchema(): Promise<void> {
    this.updateStatus(
      "Status: Sampling bookmarks to generate novel categories...",
    );
    ...
    try {
      ...
      if (charsPerItem < MIN_CHARS_PER_ITEM) {
        console.warn(
          `[BookmarkClassifier] Warning: Very strict character budget (${charsPerItem}/item). Prompt might be degraded.`,
        );
        ...
        .map((n) => {
          ...
          if (n.url) {
            ...
            if (url.length > urlLimit)
              url = url.substring(0, urlLimit - 3) + "...";
        ...
        expectedOutputs: [
          {
            type: "text",
            languages: ["en"],
            schema: GENERATE_CATEGORIES_JSON_SCHEMA,
          },
        ],
      });
      ...
      if (
        ...
        this.updateStatus(
          `Generated ${newCategories.length} novel categories | Time: ${totalTimeSeconds}s`,
        );
        this.displayResults(
          newCategories,
          "Step 0: Generate Categories - Done",
        );
        this.activeSchema = createClassificationSchema(newCategories);
      }
    } catch (error) {
      this.updateStatus(
        `Error generating categories, using default schema: ${error}`,
      );
  ...
  async createClassifications(): Promise<void> {
    ...
    this.updateStatus(
      "Status: Fetching bookmarks and starting batch classification...",
    );
    ...
    try {
      ...
      this.updateStatus(
        `Found ${bookmarksFlat.length} bookmarks. Processing in ${chunks.length} batches.`,
      );
...
`;
        ...
        try {
          ...
          console.warn(
            `[bookmark-classifier] Error processing Batch ${currentBatch}:`,
            error,
          );
      ...
      const avgBatchTime =
        batchDurations.reduce((a, b) => a + b, 0) / batchDurations.length || 0;

      this.updateStatus(
        `Classification complete | Input: ${this.originalBookmarksMap.size} | Organized: ${this.allClusteredData.length} | Total Time: ${(totalTime / 1000).toFixed(2)}s | Avg Batch: ${(avgBatchTime / 1000).toFixed(2)}s`,
      );
      this.displayResults(
        this.allClusteredData,
        "Step 1: Categorize Bookmarks - Done",
      );
      stopSpinner();
      showBadgeSuccess();
  ...
  async assignBookmarksToFolders(): Promise<void> {
    if (!this.folderHierarchy || this.allClusteredData.length === 0) {
      this.updateStatus(
        "Error: Missing data. Please run Step 1 and Step 2 first.",
      );
      return;
    }
    const stopSpinner = startExtensionSpinner();
    ...
    try {
      ...
      const clusterToFolderNode = new Map<
        string,
      ...
      for (const item of this.allClusteredData) {
        ...
        if (originalNode && targetFolder) {
          ...
          if (!targetFolder)
            console.debug(
              `Available Keys:`,
              Array.from(clusterToFolderNode.keys()),
            );
        }
      }
  ...
  updateButtonStates(): void {
    const btn0 = this.shadowRoot!.querySelector(
      "#generate-novel-schema",
    ) as HTMLButtonElement;
    const btn1 = this.shadowRoot!.querySelector(
      "#create-classifications",
    ) as HTMLButtonElement;
    const btn2 = this.shadowRoot!.querySelector(
      "#create-hierarchy",
    ) as HTMLButtonElement;
    const btn3 = this.shadowRoot!.querySelector(
      "#assign-folders",
    ) as HTMLButtonElement;
        ...
        if (node.url) {
          ...
          if (!seenContent.has(content)) {
            ...
            if (cleanUrl.length > 40)
              cleanUrl = cleanUrl.substring(0, 37) + "...";
